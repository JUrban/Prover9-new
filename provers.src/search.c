...
Prover_options init_prover_options(void)
{
  ...
  p->hint_compiled_cache_kb =
    init_parm("hint_compiled_cache_kb", 32768, 0, INT_MAX);
...
void update_memory_stats(void)
{
  ...
  Stats.dense_passive_selector_file_read_evictions =
    selectors.file_read_evictions;
  Stats.dense_passive_selector_file_read_eviction_bytes =
...
static void configure_search_indexes(void)
{
  ...
  configure_compact_back_demod_strategy(
    ...
    str_ident(stringparm1(Opt->compact_back_demod_strategy), "hot_root_tree") ?
      COMPACT_BACK_DEMOD_HOT_ROOT_TREE :
    str_ident(stringparm1(Opt->compact_back_demod_strategy), "position") ?
      COMPACT_BACK_DEMOD_POSITION :
    str_ident(stringparm1(Opt->compact_back_demod_strategy), "adaptive") ?
      COMPACT_BACK_DEMOD_ADAPTIVE : COMPACT_BACK_DEMOD_MASK8);
...
void write_checkpoint(void)
{
  ...
  /* 1. Write metadata.txt */
  {
    ...
    fprintf(fp, "compact_rewrite_subject_atoms %llu\n",
...
void resume_load_clauses(const char *dir)
{
  ...
  rewind(fp); (void) read_metadata_ull_if_present(
    fp, "compact_rewrite_subject_atoms",
    &Stats.compact_rewrite_subject_atoms);
  rewind(fp); (void) read_metadata_ull_if_present(
