...
/* Wall-clock timeout for predicate elimination. */

static time_t Pred_elim_deadline = 0;  /* 0 = no timeout */
static BOOL   Pred_elim_expired = FALSE;
static int Pred_elim_check_counter = 0;
#define PRED_ELIM_CHECK_INTERVAL 100

static
BOOL pred_elim_timed_out(void)
{
  if (Pred_elim_expired)
    return TRUE;
  if (Pred_elim_deadline == 0)
    return FALSE;
  Pred_elim_check_counter++;
  if (Pred_elim_check_counter < PRED_ELIM_CHECK_INTERVAL)
    return FALSE;
  Pred_elim_check_counter = 0;
  if (time(NULL) >= Pred_elim_deadline) {
    Pred_elim_expired = TRUE;
    return TRUE;
  }
  return FALSE;
}  /* pred_elim_timed_out */
...
BOOL eliminable_relation(int symbol, Plist clauses, BOOL equality)
{
  ...
  */
  ...
  for (p = clauses; p; p = p->next) {
    int n;
    if (pred_elim_timed_out())
      return FALSE;
    n = rsym_occurrences(symbol, p->v);
    if (n > 1)
      return FALSE;
...
Ilist eliminable_relations(Plist clauses, BOOL equality)
{
  ...
  for (p = rsyms; p; p = p->next) {
	...
	eliminable_relation(p->i, clauses, equality))
      ...
      eliminable = ilist_prepend(eliminable, p->i);
  }
  zap_ilist(rsyms);
  return reverse_ilist(eliminable);
}  /* eliminable_relations */
...
static
Plist resolve_on_symbol(int symbol, Topform c1, Topform c2, BOOL echo)
{
  ...
  for (l1 = c1->literals; l1; l1 = l1->next)
	...
	if (res) {
	  if (tautology(res->literals))
	    ...
	    resolvents = plist_prepend(resolvents, res);
	  }
	}
      }
    }
...
static
Plist gen_given(int symbol, Topform given, Plist usable, BOOL echo)
{
  Plist new = NULL;
  Plist p;
  for (p = usable; p; p = p->next) {
    Topform c = p->v;
    Plist resolvents = resolve_on_symbol(symbol, given, c, echo);
...
BOOL subsumed_by_member(Topform c, Plist p)
{
  while (p != NULL) {
    if (pred_elim_timed_out())
      return FALSE;  /* bail out: treat as not subsumed */
    if (subsumes(p->v, c))
      return TRUE;
    p = p->next;
  }
...
Plist elim_relation(int symbol, Plist sos, Clause_store disabled, BOOL echo)
{
  ...
  while (sos) {
    ...
    Topform given;
    if (pred_elim_timed_out()) {
      /* Bail out: return all clauses without completing elimination. */
      return plist_cat(usable, sos);
    }
    given = sos->v;
    sos = plist_pop(sos);
    usable = plist_prepend(usable, given);
#ifdef GEN_DEBUG
    printf("\ngiven: "); f_clause(given);
#endif
    new = gen_given(symbol, given, usable, echo);
...
{
  if (seconds > 0)
    Pred_elim_deadline = time(NULL) + (time_t)seconds;
  else
    Pred_elim_deadline = 0;
