...
def write_guide(records: list[ProofRecord], output: TextIO, source: str) -> None:
    ...
    for record in records:
        output.write(f"  {record.body}\n")
        output.write(
            f'    # label("proof_parent_node={record.source_id}")\n'
        )
        if record.parents:
            parent_text = ",".join(str(parent) for parent in record.parents)
            output.write(
                f'    # label("proof_parent_{record.rule}={parent_text}")\n'
            )
        if record.rewrite_parents:
            rewrite_text = ",".join(
                str(parent) for parent in record.rewrite_parents
            )
            output.write(
                f'    # label("proof_parent_rewrite={rewrite_text}")\n'
            )
        output.write("    .\n")
